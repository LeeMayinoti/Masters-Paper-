...
The Semantic Web stack standardizes machine-readable knowledge representation through RDF triples \cite{w3c2014rdfconcepts}, commonly serialized in the compact Turtle syntax \cite{w3c2014turtle}, with OWL providing class- and property-level ontological structure over that graph \cite{w3c2012owl2} --- the RDF/Turtle/OWL combination \texttt{jnang} uses for \texttt{folktales\_ontology.ttl}. Retrieval-Augmented Generation (RAG) grounds LLM output in externally retrieved documents \cite{lewis2020retrieval}; GraphRAG extends this by retrieving structured subgraphs rather than flat text spans, improving multi-hop and structural coherence \cite{edge2024graphrag}, an approach graph databases such as Neo4j operationalize directly for knowledge-graph-backed retrieval \cite{clemedtson2025graphraft}. Orchestration frameworks such as LangGraph \cite{langchain2024langgraph}, AutoGen \cite{wu2023autogen}, and MetaGPT \cite{hong2023metagpt} structure LLM pipelines as graphs or conversations of role-specialized agents rather than a single monolithic prompt --- the same decomposition \texttt{jnang} adopts for its Generator, Classifier, and Evaluator agents.
...
\begin{table}[h]
\caption{Deterministic (non-LLM) checks, all 8 themes.}\label{tab:deterministic}
\begin{tabular}{@{}lcc@{}}
\toprule
Metric & Baseline & \texttt{jnang} \\
\midrule
European-Trope-Free Rate & 75\% & \textbf{100\%} \\
Click-Consonant Term Verification Rate & n/a (0 glossed terms) & 36\% \\
\bottomrule
\end{tabular}
\end{table}

\texttt{jnang} out-scored the naive baseline on cultural authenticity under the independent judge in every one of the 8 themes tested (binomial sign test, $p \approx 0.004$), and the deterministic trope scanner independently confirms the direction: the naive baseline leaked literal European vocabulary (``king,'' ``gold'') in 2 of 8 stories, while \texttt{jnang} produced zero trope hits across all 8.

\textbf{Self-grading bias.} The two graders disagree substantially: mean cultural-authenticity scores differ by 1.87 points on average (in-pipeline higher in 13 of 16 rows), and the graders' \texttt{overall\_grade} labels match exactly in only 5 of 16 rows (31\%; Pearson $r=0.76$ on raw cultural-authenticity scores). The disagreement is asymmetric in a revealing way: the in-pipeline grader --- sharing the Generator's model family --- rates the \emph{baseline} arm far more leniently than the independent judge does (4.83 vs.\ 2.15), while its \texttt{jnang}-arm scores stay close to the independent judge's (7.42 vs.\ 6.36). The No Unpunished Hoarding Rate makes this concrete: the in-pipeline grader reports an identical 62\% for both arms, effectively blind to the difference our own rubric is designed to detect, while the independent judge separates them sharply (38\% vs.\ 88\%). This is direct evidence that grading a system with a same-family, non-blind judge risks inflating exactly the baseline it should be compared against, motivating the independent-judge design used here.

\textbf{Generalization.} \texttt{jnang} also outperformed the baseline on all 3 held-out themes under the independent judge, though by a smaller margin than on the 5 presets (cultural authenticity: preset baseline 2.37 $\rightarrow$ \texttt{jnang} 7.81; held-out baseline 1.78 $\rightarrow$ \texttt{jnang} 3.96). The system's advantage is not confined to its 4 ontology-formalized stories, but it is weaker where RAG grounding is necessarily indirect.

\textbf{Orthographic precision is a genuine limitation, not a strength.} Only 36\% of \texttt{jnang}'s click-consonant vocabulary glosses verified against the reference lexicon (the baseline produced no glossed terms to check at all). Tracing the unverified terms to their source shows two concrete causes: the Generator's own hardcoded kinship vocabulary (e.g.\ \texttt{!ga} for grandmother, \texttt{!o} for uncle) does not match the headwords actually present in \texttt{ju\_dictionary.xls}, and for concepts outside its fixed 20-row vocabulary sample (e.g.\ tortoise, ostrich) the Generator invents plausible-sounding but unverified click-consonant terms rather than retrieving real ones. Both are addressable --- validating hardcoded prompt vocabulary against the reference lexicon, and expanding or theme-matching the vocabulary sample --- but the current figure should not be read as evidence of orthographic fidelity.
...
\textbf{Limitations.} Our evaluation is pilot-scale: $N=16$ narratives with no repeats per cell, so the results in Section~\ref{sec:results} are directionally consistent (8 of 8 themes favor \texttt{jnang} under blind judging) rather than statistically powered. LLM-based grading itself carries real reliability limits --- our two graders agreed on an overall grade label only 31\% of the time --- and even our independent judge is still an LLM, not a \juhoansi{} cultural authority; no human or community evaluation was conducted. The RDF ontology and RAG pool currently formalize only 4 of the 14 stories in the source corpus, and orthographic precision (Section~\ref{sec:results}) shows the Generator's hardcoded vocabulary is not fully validated against the reference lexicon. Ongoing work includes expanding community evaluations in Namibia, incorporating \juhoansi{} speakers as graders, formalizing the remaining source stories, and extending the framework to related Khoisan and Bantu oral traditions.
