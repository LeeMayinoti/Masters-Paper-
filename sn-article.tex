...
% ]
%
% \node[term] (start) at (0,4.2) {start};
% \node[dec] (router) at (0,3.2) {input\_story\\provided?};
%
% \node[box] (echo) at (-2.6,2.1) {echo\_input\_story};
% \node[box] (gen) at (2.6,2.1) {Agent 1: Generator};
%
% \node[box] (cls) at (0,1.1) {Agent 2: Classifier};
% \node[box] (grd) at (0,0.2) {Agent 3: Grader};
% \node[term] (end) at (0,-0.7) {end};
%
% \draw[arr] (start) -- (router);
%
% \draw[arr] (router) -| 
%     node[pos=0.3, above, font=\tiny] {yes} (echo);
%
% \draw[arr] (router) -| 
%     node[pos=0.3, above, font=\tiny] {no} (gen);
%
% \draw[arr] (echo) |- (cls);
% \draw[arr] (gen) |- (cls);
%
...
box/.style={rectangle, draw, rounded corners, minimum width=2.6cm,
            minimum height=0.9cm, align=center, fill=gray!8, font=\footnotesize},
term/.style={rectangle, draw, rounded corners, minimum width=1.4cm,
             minimum height=0.9cm, align=center, font=\footnotesize},
dec/.style={diamond, draw, aspect=2, align=center, fill=gray!8,
            font=\scriptsize, inner sep=1pt},
arr/.style={-{Latex[length=2mm]}, thick}
]
\node[term] (start)  at (0,0)      {start};
\node[dec]  (router) at (2.9,0)    {input\_story\\provided?};
\node[box]  (echo)   at (6.6,1.3)  {echo\_input\_story};
\node[box]  (gen)    at (6.6,-1.3) {Agent 1: Generator};
\node[box]  (cls)    at (10.1,0)   {Agent 2: Classifier};
\node[box]  (grd)    at (13.2,0)   {Agent 3: Grader};
\node[term] (end)    at (15.4,0)   {end};

\draw[arr] (start) -- (router);
\draw[arr] (router) |- node[pos=0.75, above, font=\tiny] {yes} (echo);
\draw[arr] (router) |- node[pos=0.75, below, font=\tiny] {no} (gen);
\draw[arr] (echo) -| (cls);
\draw[arr] (gen)  -| (cls);
\draw[arr] (cls) -- (grd);
\draw[arr] (grd) -- (end);
...
\noindent\textbf{Sample output.} The following excerpt is from the \texttt{jnang} narrative for the trickster and
waterhole theme:
